...
class Leilao(Base):
    ...
    vencedor_id: Mapped[uuid.UUID | None] = mapped_column(Uuid(native_uuid=True), ForeignKey("compradores.id"))
...
class Lance(Base):
    ...
    leilao_id: Mapped[uuid.UUID] = mapped_column(
        Uuid(native_uuid=True), ForeignKey("leiloes.id"), nullable=False
    )
